...
class HomeMenu(ctk.CTkFrame):
    def __init__(self, master, show_specs_cmd, show_optimize_cmd, master_app):
        ...
        # PLAY
        self.play_btn = ctk.CTkButton(container, text="[ PLAY ]", font=styles.FONT_ORBITRON_MD,
                                      fg_color="transparent", border_width=2, border_color="#ff4655",
                                      height=250, width=380, command=lambda: launcher.start_game(master_app.show_dashboard))
        styles.apply_tactical_style(self.play_btn)
        self.play_btn.pack(side="left", padx=20)

        ctk.CTkLabel(container, text="/", font=("Impact", 120),
        ...
        self.opt_btn = ctk.CTkButton(footer, text="[ OPTIMIZE ]", font=styles.FONT_ORBITRON_SM,
                                     fg_color="#ff4655", height=75, width=280, command=show_optimize_cmd)
        styles.apply_tactical_style(self.opt_btn)
        self.opt_btn.pack(pady=(10, 0))
